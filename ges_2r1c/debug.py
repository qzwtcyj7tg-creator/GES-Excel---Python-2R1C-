...
class ValidierungsExcelErsteller:
    ...
    def _neueste_exportdatei_finden(self) -> Path:
        ...
        muster = [
            'Zuluft_Ergebnisse_*.xlsx', 'Zuluft_Ergebnisse_*.xls',
            '*Zuluft*Ergebnisse*.xlsx', '*Zuluft*Ergebnisse*.xls',
        ]
        kandidaten = []
        for m in muster:
    ...
    def _aufbauen(self, df_py: pd.DataFrame, df_ref: pd.DataFrame) -> pd.DataFrame:
        ...
        result['Außentemperatur [°C]'] = (
            df_py['Außentemperatur [°C]'] if 'Außentemperatur [°C]' in df_py.columns
            else df_ref['Außentemperatur [°C]']
        )
    ...
    def _formatieren(self, datei: Path):
        ...
        light_orange_fill = PatternFill(fill_type='solid', start_color='FFF2CC', end_color='FFF2CC')
